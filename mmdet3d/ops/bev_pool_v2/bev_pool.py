...
def bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
    ...
    if bev_pool_v2_ext is None:
        ...
        feat_flat = feat.reshape(-1, feat_channels)
        pooled = depth_flat[ranks_depth].unsqueeze(1) * feat_flat[ranks_feat]

        out = feat.new_zeros(
            (int(np.prod(bev_feat_shape[:-1])), feat_channels))
        out.index_add_(0, ranks_bev, pooled)
        x = out.view(*bev_feat_shape)
    else:
        x = QuickCumsumCuda.apply(depth, feat, ranks_depth, ranks_feat,
